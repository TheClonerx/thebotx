...
#  Where a more satisfactory response to all bug reports (which can be made on
#  the issues page) and other statements can be garnered. For things not
#  specific or in direct reference to this library, 'ch.py', a direct response
#  can be filed to the individual persons listed above as 'Current Maintainers
#  and Contributors.'
################################################################
...
def _parseFont(f):
  ...
  #' xSZCOL="FONT"'
  match = re.match(r' x(?P<size>\d{2})(?P<color>[0-9a-fA-F]{3,6})="(?P<font>.*)"', f)
  if not match:
    return None, None, None
  else:
    groups = match.groupdict()
    color  = groups["color"] if "color" in groups else None
    font   = groups["font"]  if "font"  in groups else None
    size   = groups["size"]  if "size"  in groups else None
